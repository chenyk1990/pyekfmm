...
receiverx=501
receiverz=501
paths,nrays=fmm.stream2d(-tx,-tz, receiverx, receiverz, step=0.1, maxvert=10000)
print('Before trim',paths.shape)
## trim the rays and add the source point
paths=fmm.trimrays(paths,start_points=np.array([1,1]),T=0.5)
...
plt.plot((receiverx-1)*dx,(receiverz-1)*dz,'vb',markersize=15);
## plot rays
plt.plot((paths[0,:]-1)*dx,(paths[1,:]-1)*dz,'g--',markersize=20);

plt.savefig('test_1_vgrad_oneray.png',format='png',dpi=300,bbox_inches='tight', pad_inches=0)
plt.savefig('test_1_vgrad_oneray.pdf',format='png',dpi=300,bbox_inches='tight', pad_inches=0)
